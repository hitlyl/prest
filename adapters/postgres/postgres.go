			...
			for k, v := range val {
				...
				if len(keyInfo) > 1 {
					...
					case "tsquery-opt":
						...
						if len(tsQueryField) == 2 {
							tsQuery = fmt.Sprintf(`%s @@ to_tsquery('%s', array_to_string(tsvector_to_array(to_tsvector('%s','%s')),'|'))`, tsQueryField[0], tsQueryField[1], tsQueryField[1], value)
						}
						if len(tsQueryField) == 3 {
							tsQuery = fmt.Sprintf(`%s @@ to_tsquery('%s',array_to_string(tsvector_to_array(to_tsvector('%s','%s')),'%s'))`, tsQueryField[0], tsQueryField[1], tsQueryField[1], value, tsQueryField[2])
						}
						whereKey = append(whereKey, tsQuery)
					default:
